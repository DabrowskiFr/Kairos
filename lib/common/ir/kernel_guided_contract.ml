...
let exported_summary_of_exported_ir
    (summary : Proof_kernel_ir.exported_node_summary_ir) : exported_summary_contract =
  {
  ...
  let symbolic_groups =
    {
      source_product_summaries =
        List.filter
          (fun clause -> clause.Proof_kernel_ir.origin = Proof_kernel_ir.OriginSourceProductSummary)
          obligations.symbolic;
      phase_steps =
        List.filter
          (fun clause -> clause.Proof_kernel_ir.origin = Proof_kernel_ir.OriginPhaseStepSummary)
          obligations.symbolic;
      propagation =
        List.filter
          (fun clause ->
            clause.Proof_kernel_ir.origin = Proof_kernel_ir.OriginPropagationNodeInvariant
            || clause.Proof_kernel_ir.origin = Proof_kernel_ir.OriginPropagationAutomatonCoherence)
          obligations.symbolic;
      safety =
        List.filter
          (fun clause -> clause.Proof_kernel_ir.origin = Proof_kernel_ir.OriginSafety)
          obligations.symbolic;
    }
  in
  {
