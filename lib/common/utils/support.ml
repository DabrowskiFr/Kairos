...
let term_var (env : env) (x : ident) : Ptree.term_desc =
  if is_rec_var env x then Tident (qdot (qid1 env.rec_name) (rec_var_name env x))
  else Tident (qid1 x)
...
let rec max_x_depth (f : fo_ltl) : int =
  ...
  | LAnd (a, b) | LOr (a, b) | LImp (a, b) -> max (max_x_depth a) (max_x_depth b)
...
let normalize_ltl_for_k ~(init_for_var : ident -> iexpr) (f : fo_ltl) : ltl_norm =
  let rec shift_ltl_with_depth k depth f =
    ...
    | LTrue | LFalse -> Some f
    | LNot a -> begin
        match shift_ltl_with_depth k depth a with Some a' -> Some (LNot a') | None -> None
      end
    | LAnd (a, b) -> begin
    ...
    | LG a -> begin
        match shift_ltl_with_depth k depth a with Some a' -> Some (LG a') | None -> None
      end
    | LAtom (FRel (h1, r, h2)) ->
        let shift = k - depth in
        begin match
          (shift_hexpr_by ~init_for_var shift h1, shift_hexpr_by ~init_for_var shift h2)
        with
    ...
    | LAtom (FPred (id, hs)) ->
        ...
        let rec map acc = function
          ...
          | h :: rest -> (
              ...
              | None -> None)
        in
        begin match map [] hs with Some hs' -> Some (LAtom (FPred (id, hs'))) | None -> None
        end
  in
...
let rec shift_ltl_by ~(init_for_var : ident -> iexpr) (shift : int) (f : fo_ltl) : fo_ltl option =
  if shift <= 0 then Some f
    ...
    | LTrue | LFalse -> Some f
    | LNot a -> begin
        match shift_ltl_by ~init_for_var shift a with Some a' -> Some (LNot a') | None -> None
      end
    | LAnd (a, b) -> begin
    ...
    | LG a -> begin
        match shift_ltl_by ~init_for_var shift a with Some a' -> Some (LG a') | None -> None
      end
