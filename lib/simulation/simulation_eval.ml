...
let eval_error msg = Error (Pipeline_api_types.Stage_error ("eval: " ^ msg))
...
let parse_typed_value ~(name : string) ~(ty : Ast.ty) (raw : string) :
    (eval_value, Pipeline_api_types.error) result =
  match ty with
  | Ast.TInt -> (
...
let eval_iexpr (env : (string, eval_value) Hashtbl.t) (e : Ast.iexpr) :
    (eval_value, Pipeline_api_types.error) result =
  let rec go (e : Ast.iexpr) : (eval_value, Pipeline_api_types.error) result =
    match e.iexpr with
    | ILitInt i -> Ok (VInt i)
...
let eval_guard env (g : Ast.iexpr option) : (bool, Pipeline_api_types.error) result =
  match g with
  | None -> Ok true
  | Some e -> (
...
let rec eval_stmt_list (env : (string, eval_value) Hashtbl.t) (stmts : Ast.stmt list) :
    (unit, Pipeline_api_types.error) result =
  match stmts with
  | [] -> Ok ()
  | s :: rest -> (
      match s.stmt with
...
let parse_trace_for_node ~(n : Ast.node) (trace_text : string) :
    ((string * eval_value) list list, Pipeline_api_types.error) result =
  let input_types =
    List.fold_left
      (fun m vd ->
...
let eval_pass ~input_file ~trace_text ~with_state ~with_locals :
  ...
  try
    let p = Parse_api.parse_file input_file in
    let n =
      match p with
      | [ n ] -> Ok n
    ...
    | Ok n -> (
        ...
        | Ok steps ->
            ...
            let transitions_from_state = Ast_queries.transitions_from_state_fn n in
            let lines = ref [] in
            ...
            | Error _ as err -> err
            | Ok () -> Ok (String.concat "\n" (List.rev !lines)))
  with exn -> Error (Pipeline_api_types.Stage_error ("eval: " ^ Printexc.to_string exn))
