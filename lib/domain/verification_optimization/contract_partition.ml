...
(** Contract partitioning is an optional proof-planning optimization, not part
    of the reference semantics.

    Every generated group is a conjunction of source guarantees, and the groups
    must cover every source guarantee. Together with the fact that groups only
    ...
    [Weak_until] splits contracts before automata generation to avoid the
    combinatorial cost of conjoined weak-until properties.

    Internal auxiliary non-W guarantees are copied into every W group to retain
    useful deductive context. Public non-W guarantees may be grouped by temporal
    family and conclusion footprint. These are proof-cost choices; neither
    changes the property being verified. *)
...
let build_groups ~(public_non_w : public_non_w_strategy)
...
let partition_node ~(public_non_w : public_non_w_strategy)
  ...
  | [] | [ _ ] -> Ok [ identity_spec node ]
  | guarantees ->
      let refs = guarantee_refs node in
      let groups =
        build_groups ~public_non_w refs
      in
      ...
      if List.length groups = 1 then
        ...
        |> List.mapi (fun idx group ->
               ({
                  ...
                  guarantee_indices = group.members;
                }
                 : Proof_case_program.case_spec))
        |> fun specs -> Ok specs

let apply_weak_until ~(public_non_w : public_non_w_strategy)
  ...
  let rec loop specs_acc = function
    ...
    | node :: rest -> (
        match partition_node ~public_non_w ~used_names node with
        | Error _ as err -> err
...
let apply ~(strategy : strategy) (proof_cases : Proof_case_program.t) :
  ...
  | Weak_until { public_non_w } ->
      apply_weak_until ~public_non_w proof_cases
