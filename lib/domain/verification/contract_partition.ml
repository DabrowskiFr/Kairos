...
(** Contract partitioning is a proof-structuring pass, not a contract
    rewriting pass.
    ...
    grouping is controlled by an explicit domain policy. *)
...
let build_groups ~(policy : policy)
  ...
  if not (List.exists (fun g -> g.has_weak_until) refs) then
    ...
    let public_non_w_groups =
      if policy.group_public_non_w_guarantees then
...
let fresh_reference_name used_names base group_index =
  let rec loop suffix =
...
let partition_node ~(policy : policy)
  ...
  | guarantees ->
      ...
      let groups = build_groups ~policy refs in
        ...
        |> List.mapi (fun idx group ->
               {
                 node with
                 node_name =
                   fresh_reference_name used_names node.node_name (idx + 1);
                 guarantees = group_formulas ~source group;
               })
        |> fun nodes -> Ok nodes

let partition_program ~(policy : policy)
    (program : Verification_model.program_model) :
  ...
  List.iter
    ...
    program;
  let rec loop nodes_acc origins_acc = function
    | [] ->
        Ok
          {
            program = List.rev nodes_acc;
            provenance = List.rev origins_acc;
          }
    | node :: rest -> (
        match partition_node ~policy ~used_names node with
        | Error _ as err -> err
        | Ok nodes ->
            let origins =
              List.map
                (fun (reference_node : Verification_model.node_model) ->
                  {
                    reference_node_name = reference_node.node_name;
                    source_node_name = node.node_name;
                  })
                nodes
            in
            loop
              (List.rev_append nodes nodes_acc)
              (List.rev_append origins origins_acc)
              rest)
  in
  loop [] [] program
