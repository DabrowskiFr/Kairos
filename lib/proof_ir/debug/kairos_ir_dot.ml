...
let dot_of_annotated_transition (t : Kairos_ir.annotated_transition) : string =
  let raw = t.raw in
  let guard_str = truncate (Support.string_of_ltl raw.guard) |> html_escape in
...
let dot_of_annotated_node (n : Kairos_ir.annotated_node) : string =
  let raw = n.raw in
...
let dot_of_verified_transition (t : Kairos_ir.verified_transition) : string =
  let guard_str = truncate (Support.string_of_ltl t.guard) |> html_escape in
  let buf = Buffer.create 256 in
  Buffer.add_string buf
    (Printf.sprintf
       "  %s -> %s [label=<\n    <TABLE BORDER=\"0\" CELLPADDING=\"2\" CELLSPACING=\"0\">\n"
  ...
  if t.pre_k_updates <> [] then begin
    let upd_strs =
      List.map
        (fun (s : Ast.stmt) ->
          match s.stmt with
          | SAssign (v, e) -> truncate (v ^ " := " ^ Support.string_of_iexpr e) |> html_escape
          | _ -> "...")
        t.pre_k_updates
    in
    List.iter
...
let dot_of_verified_node (n : Kairos_ir.verified_node) : string =
  let buf = Buffer.create 1024 in
  Buffer.add_string buf
    (Printf.sprintf
       "digraph \"%s\" {\n  rankdir=LR;\n  node [shape=circle fontname=\"Courier\" fontsize=10];\n  edge [fontname=\"Courier\" fontsize=9];\n  // States\n"
...
(* ------------------------------------------------------------------ *)
(* kernel node_ir (Product_kernel_ir.node_ir)                          *)
(* ------------------------------------------------------------------ *)

let product_state_label (ps : Product_kernel_ir.product_state_ir) : string =
  Printf.sprintf "%s_A%d_G%d" ps.prog_state ps.assume_state_index ps.guarantee_state_index

let dot_of_kernel_node_ir (n : Product_kernel_ir.node_ir) : string =
  ...
  List.iter
    (fun (ps : Product_kernel_ir.product_state_ir) ->
  ...
  List.iter
    (fun (step : Product_kernel_ir.product_step_ir) ->
      ...
      let guard_str =
        truncate (Support.string_of_ltl step.program_guard) |> String.escaped
      in
      let color =
