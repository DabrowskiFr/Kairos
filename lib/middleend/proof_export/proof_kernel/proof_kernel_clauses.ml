...
let build_source_summary_clauses ~(node : Abs.node) ~(analysis : Product_build.analysis)
  ...
  let source_summaries =
    ...
    |> List.filter_map (fun (st : product_state_ir) ->
           ...
           let phase_formula =
             ...
             | [] -> None
             | f :: rest ->
                 Some (List.fold_left (fun acc fo_atom -> LOr (acc, fo_atom)) f rest |> normalize_source_summary)
           in
  ...
  let phase_formula_of_clause (clause : generated_clause_ir) =
    ...
    |> List.find_map (fun (fact : clause_fact_ir) ->
           ...
           | CurrentTick, FactPhaseFormula fo_atom -> Some fo_atom
           | _ -> None)
  in
    ...
    List.concat_map
      (fun step ->
          ...
          if src_live then
            let base_hypotheses =
              [
                ...
                step_ctx (FactFormula (ltl_of_fo step.program_guard));
                step_ctx (FactFormula (ltl_of_fo step.assume_edge.guard));
              ]
            in
            let phase_clause =
              [
                ({
                  ...
                  conclusions = [ current (FactPhaseFormula (ltl_of_fo step.guarantee_edge.guard)) ];
                } : generated_clause_ir);
              ]
            in
        ...
        let safety =
          match step.step_kind with
          | StepBadGuarantee ->
              split_top_level_or (ltl_of_fo step.guarantee_edge.guard)
...
let lower_clause_fact ~(pre_k_map : (Ast.hexpr * Temporal_support.pre_k_info) list) (fact : clause_fact_ir) :
  ...
  let lower_desc = function
    ...
    | FactPhaseFormula fo_atom ->
        Option.map (fun fo_atom' -> FactPhaseFormula fo_atom') (lower_ltl_temporal_bindings ~temporal_bindings fo_atom)
    | FactFalse -> Some FactFalse
    | FactFormula fo_atom ->
        Option.map (fun fo_atom' -> FactFormula fo_atom') (lower_ltl_temporal_bindings ~temporal_bindings fo_atom)
  in
...
let relationalize_clause_fact ~(pre_k_map : (Ast.hexpr * Temporal_support.pre_k_info) list)
  ...
  let rel_desc = function
    ...
    | FactPhaseFormula fo_atom ->
        Option.map (fun fo_atom' -> RelFactPhaseFormula fo_atom') (lower_ltl_temporal_bindings ~temporal_bindings fo_atom)
    | FactFormula fo_atom ->
        Option.map (fun fo_atom' -> RelFactFormula fo_atom') (lower_ltl_temporal_bindings ~temporal_bindings fo_atom)
...
let expand_relational_hypotheses (facts : relational_clause_fact_ir list) :
  ...
  let rec expand_one acc = function
    ...
    | ({ desc = RelFactFormula (LOr (a, b)); _ } as fact) :: tl ->
        let left = { fact with desc = RelFactFormula (Fo_simplifier.simplify_ltl a) } in
        let right = { fact with desc = RelFactFormula (Fo_simplifier.simplify_ltl b) } in
...
let normalize_relational_hypotheses (facts : relational_clause_fact_ir list) :
  ...
  let combine_formula left right =
    ...
    | RelFactFormula a, RelFactFormula b -> Some (RelFactFormula (Fo_simplifier.simplify_ltl (LAnd (a, b))))
    | _ -> None
  in
