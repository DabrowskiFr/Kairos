...
(** Pair raw transitions with their corresponding abstract transitions.
    ...
    pass and in [Raw_obligation_generation.build_raw_node]). *)
let zip_transitions (raws : Proof_obligation_ir.raw_transition list)
  ...
  with Invalid_argument _ ->
    failwith
      "Triple_annotation.annotate: raw transitions and abstract transitions \
...
(** Annotate a raw node with Hoare triples.
    ...
    [Normalized_instrumentation.finalize_instrumented_abstract_node]: its transitions
    already carry the requires/ensures generated by the abstract post/pre
    phases. *)
