...
let guard_true (atom_names : string list) : guard =
  [ List.map (fun name -> (name, None)) atom_names ]

let merge_guards (g1 : guard) (g2 : guard) : guard = prime_implicants (g1 @ g2)

let ordered_state_ids ~(start : int) (states : hoa_state list) : int list =
  let ids = List.map (fun st -> st.id) states |> List.sort_uniq compare in
  if List.mem start ids then start :: List.filter (( <> ) start) ids else ids

let normalize_spot_automaton ~(atom_names : string list) (hoa : hoa_automaton) : automaton =
  let by_id = Hashtbl.create (List.length hoa.states * 2) in
  List.iter (fun st -> Hashtbl.replace by_id st.id st) hoa.states;
  let rejecting =
    hoa.states |> List.filter (fun st -> not st.accepting) |> List.map (fun st -> st.id) |> List.sort_uniq compare
  in
  let has_bad = rejecting <> [] in
  let accepting_ids =
    hoa.states |> List.filter (fun st -> st.accepting) |> List.map (fun st -> st.id) |> List.sort_uniq compare
  in
  ...
  let add_transition src guard dst =
    ...
    Hashtbl.replace table key (merge_guards prev guard)
  in
  List.iter
    (fun st ->
      if not (has_bad && List.mem st.id rejecting) then
        ...
        List.iter
          ...
          st.transitions)
    hoa.states;
  if has_bad then add_transition bad_idx (guard_true atom_names) bad_idx;
  let transitions =
...
let build ~(atom_map : (fo * ident) list) ~(atom_names : ident list) (spec : fo_ltl) : automaton =
  ...
  normalize_spot_automaton ~atom_names hoa
