...
let build_contracts_runtime_view ~(nodes : Ast.node list) ?kernel_ir (info : Why_env.env_info)
  ...
  let kernel_product_coverage =
    ...
    | Some ir -> ir.product_coverage <> Proof_kernel_types.CoverageEmpty && not !pure_translation
    | None -> false
  in
  ...
  let kernel_clause_origin_label = function
    | Proof_kernel_types.OriginSourceProductSummary -> "Kernel source product summary"
    | Proof_kernel_types.OriginPhaseStepPreSummary -> "Kernel phase step pre summary"
    | Proof_kernel_types.OriginPhaseStepSummary -> "Kernel phase step summary"
  ...
  in
  let compile_tick_ctx_fo (f : Ast.fo_atom) : Ptree.term =
    match f with
    | FRel (h1, r, h2) ->
        mk_term
          (Tinnfix
  ...
  let source_summary_formula_for =
    ...
    | Some contract ->
        ...
        let extract_state = function
          | Proof_kernel_types.
              { time = Proof_kernel_types.CurrentTick; desc = Proof_kernel_types.RelFactProgramState st }
            ->
              Some st
          | _ -> None
        in
        let extract_gstate = function
          | Proof_kernel_types.
              {
                time = Proof_kernel_types.CurrentTick;
                desc = Proof_kernel_types.RelFactGuaranteeState idx;
              } ->
              Some idx
          | _ -> None
        in
        let extract_formula = function
          | Proof_kernel_types.
              { time = Proof_kernel_types.CurrentTick; desc = Proof_kernel_types.RelFactFormula fo_atom } ->
              Some fo_atom
          | _ -> None
        in
        List.iter
          (fun (clause : Proof_kernel_types.relational_generated_clause_ir) ->
            ...
            | Proof_kernel_types.OriginSourceProductSummary ->
                ...
                begin
                  ...
                  | Some st, Some g, Some fo_atom -> Hashtbl.replace table (st, g) fo_atom
                  | _ -> ()
                end
            | _ -> ())
  ...
  let step_phase_formula_for =
    ...
    | Some contract ->
        ...
        let extract_formula = function
          | Proof_kernel_types.
              { time = Proof_kernel_types.CurrentTick; desc = Proof_kernel_types.RelFactPhaseFormula fo_atom }
            ->
              Some fo_atom
          | _ -> None
        in
        List.iter
          (fun (clause : Proof_kernel_types.relational_generated_clause_ir) ->
            ...
            | Proof_kernel_types.OriginPhaseStepSummary, ClauseAnchorProductStep step -> begin
                match List.find_map extract_formula clause.conclusions with
                | Some fo_atom -> Hashtbl.replace table step fo_atom
                | None -> ()
              end
            | _ -> ())
          contract.obligations.symbolic;
  ...
  let step_phase_pre_formula_for =
    ...
    | Some contract ->
        ...
        let extract_formula = function
          | Proof_kernel_types.
              {
                time = Proof_kernel_types.PreviousTick;
                desc = Proof_kernel_types.RelFactPhaseFormula fo_atom;
              } ->
              Some fo_atom
          | _ -> None
        in
        List.iter
          (fun (clause : Proof_kernel_types.relational_generated_clause_ir) ->
            match (clause.origin, clause.anchor) with
            | Proof_kernel_types.OriginPhaseStepPreSummary, ClauseAnchorProductStep step -> begin
                match List.find_map extract_formula clause.conclusions with
                | Some fo_atom -> Hashtbl.replace table step fo_atom
                | None -> ()
              end
  ...
  in
  let phase_case_named_term ~(time : Proof_kernel_types.clause_time_ir) (pred_name : string) :
      Ptree.term =
    match time with
    | Proof_kernel_types.CurrentTick -> phase_case_pred_term pred_name
    | Proof_kernel_types.PreviousTick -> term_old (phase_case_pred_term pred_name)
    | Proof_kernel_types.StepTickContext -> phase_case_pred_term pred_name
  in
  let compile_relational_kernel_fact
      ?anchor_step
      ~(clause_facts : Proof_kernel_types.relational_clause_fact_ir list)
      (fact : Proof_kernel_types.relational_clause_fact_ir) : Ptree.term option =
    let compile_desc time = function
      | Proof_kernel_types.RelFactProgramState state_name ->
          ...
          Some
            ...
            | Proof_kernel_types.StepTickContext -> base)
      | Proof_kernel_types.RelFactGuaranteeState idx ->
          let paired_prog_state =
            clause_facts
            |> List.find_map (fun (other : Proof_kernel_types.relational_clause_fact_ir) ->
                   match (other.time, other.desc) with
                   | t, Proof_kernel_types.RelFactProgramState st when t = time -> Some st
                   | _ -> None)
          in
          let compile_phase_formula fo_atom =
            match time with
            | Proof_kernel_types.CurrentTick ->
                compile_ltl_term_shift env 1 (normalize_source_summary_fo fo_atom)
            | Proof_kernel_types.PreviousTick ->
                compile_ltl_term_shift env 0 (normalize_source_summary_fo fo_atom)
            | Proof_kernel_types.StepTickContext ->
                compile_ltl_term_shift ~in_post:true env 0 (normalize_source_summary_fo fo_atom)
          in
          begin
            ...
            | Some prog_state -> begin
                let step_phase_formula =
                  ...
                  | Some (step : Proof_kernel_types.product_step_ir)
                    when (time = Proof_kernel_types.CurrentTick
                         || time = Proof_kernel_types.StepTickContext)
                         && String.equal prog_state step.dst.prog_state
                         && idx = step.dst.guarantee_state_index ->
                      step_phase_formula_for ~step
                  | Some (step : Proof_kernel_types.product_step_ir)
                    when time = Proof_kernel_types.PreviousTick
                  ...
                  | Some _ ->
                      Some
                        (phase_case_named_term ~time
                           (Proof_kernel_naming.phase_state_case_name ~prog_state ~guarantee_state:idx))
                  | None -> None
                in
      ...
      | Proof_kernel_types.RelFactPhaseFormula fo_atom ->
          let base = compile_ltl_term_shift env 1 (normalize_source_summary_fo fo_atom) in
          Some
            (match time with
            | Proof_kernel_types.CurrentTick -> base
            | Proof_kernel_types.PreviousTick -> old_if_needed env base
            | Proof_kernel_types.StepTickContext ->
                compile_ltl_term_shift ~in_post:true env 0 (normalize_source_summary_fo fo_atom))
      | Proof_kernel_types.RelFactFormula fo_atom ->
          let base = compile_ltl_term_shift env 1 fo_atom in
          Some
            (match time with
            | Proof_kernel_types.CurrentTick -> base
            | Proof_kernel_types.PreviousTick -> old_if_needed env base
            | Proof_kernel_types.StepTickContext -> compile_ltl_term_shift ~in_post:true env 0 fo_atom)
      | Proof_kernel_types.RelFactFalse -> Some (mk_term Tfalse)
    in
    compile_desc fact.time fact.desc
  in
  let compile_relational_kernel_clause_summary
      ~(idx : int) ~(label : string) (clause : Proof_kernel_types.relational_generated_clause_ir) :
      compiled_kernel_clause option =
    let anchor_step =
      match clause.anchor with
      | Proof_kernel_types.ClauseAnchorProductStep step -> Some step
      | Proof_kernel_types.ClauseAnchorProductState _ -> None
    in
    let clause_source_state =
      clause.hypotheses
      |> List.find_map (fun (fact : Proof_kernel_types.relational_clause_fact_ir) ->
             match (fact.time, fact.desc) with
             | Proof_kernel_types.PreviousTick, Proof_kernel_types.RelFactProgramState state_name ->
                 Some state_name
             | Proof_kernel_types.CurrentTick, Proof_kernel_types.RelFactProgramState state_name ->
                 Some state_name
             | _ -> None)
    in
    let premise =
  ...
  let compile_merged_relational_kernel_clause_summary ~(idx : int) ~(label : string)
      (clauses : Proof_kernel_types.relational_generated_clause_ir list) :
      compiled_kernel_clause option =
    match clauses with
    | [] -> None
    | first_clause :: _ ->
        ...
        let conclusion =
          clauses
          |> List.concat_map
               (fun (clause : Proof_kernel_types.relational_generated_clause_ir) -> clause.conclusions)
          |> List.filter_map
        ...
        Option.map
          ...
          conclusion
  in
  let same_rel_step_clause_shape (a : Proof_kernel_types.relational_generated_clause_ir)
      (b : Proof_kernel_types.relational_generated_clause_ir) : bool =
    ...
    | Some contract ->
        if contract.obligations.symbolic = [] && contract.proof_step_contracts = [] then
          ...
          let propagation_safety_and_phase =
            ...
            |> List.concat_map (fun (proof_step : Proof_kernel_types.proof_step_contract_ir) ->
                   proof_step.clauses)
          in
          let rec consume_rel acc = function
            | [] -> acc
            | (idx_node, (node_clause : Proof_kernel_types.relational_generated_clause_ir))
              :: (_, (safety_clause : Proof_kernel_types.relational_generated_clause_ir))
              :: rest
              when node_clause.origin = Proof_kernel_types.OriginPropagationNodeInvariant
                   && safety_clause.origin = Proof_kernel_types.OriginSafety
                   && same_rel_step_clause_shape node_clause safety_clause ->
                let acc =
                  match
                    compile_relational_kernel_clause_summary ~idx:idx_node
                      ~label:(kernel_clause_origin_label Proof_kernel_types.OriginSafety)
                      safety_clause
                  with
                  | None -> acc
                  | Some clause ->
            ...
            | (idx_node, (node_clause : Proof_kernel_types.relational_generated_clause_ir)) :: rest
              when node_clause.origin = Proof_kernel_types.OriginPropagationNodeInvariant ->
                let acc =
                ...
                in
                consume_rel acc rest
            | (idx, (clause : Proof_kernel_types.relational_generated_clause_ir)) :: rest ->
                let compiled =
                  match clause.origin with
                  | Proof_kernel_types.OriginPropagationNodeInvariant
                  | Proof_kernel_types.OriginPropagationAutomatonCoherence
                  | Proof_kernel_types.OriginSafety ->
                      compile_relational_kernel_clause_summary ~idx
                        ~label:(kernel_clause_origin_label clause.origin)
                        clause
                  | Proof_kernel_types.OriginSourceProductSummary
                  | Proof_kernel_types.OriginPhaseStepPreSummary
                  | Proof_kernel_types.OriginPhaseStepSummary
                  | Proof_kernel_types.OriginInitNodeInvariant
                  | Proof_kernel_types.OriginInitAutomatonCoherence -> None
                in
                let acc =
                  match compiled with
                  | None -> acc
    ...
    | Some contract ->
        let from_step_entry_clauses =
          if contract.obligations.symbolic = [] && contract.proof_step_contracts = [] then
            ...
            contract.proof_step_contracts
            |> List.fold_left
                 (fun (terms, labeled, states) (proof_step : Proof_kernel_types.proof_step_contract_ir) ->
                   proof_step.entry_clauses
                   |> List.fold_left
                        (fun (terms, labeled, states)
                             (clause : Proof_kernel_types.relational_generated_clause_ir) ->
                          let premise =
                            clause.hypotheses
                            |> List.filter_map
                          ...
                          let conclusion =
                            clause.conclusions
                            |> List.filter_map (fun (fact : Proof_kernel_types.relational_clause_fact_ir) ->
                                   match fact.desc with
                                   | Proof_kernel_types.RelFactFormula fo_atom ->
                                       let normalized = normalize_source_summary_fo fo_atom in
                                       let fact =
                                         {
                                           fact with
                                           desc = Proof_kernel_types.RelFactFormula normalized;
                                         }
                                       in
                                       compile_relational_kernel_fact
                                         ~clause_facts:clause.conclusions fact
                                   | Proof_kernel_types.RelFactPhaseFormula _ ->
                                       compile_relational_kernel_fact
                                         ~clause_facts:clause.conclusions fact
                                   | _ ->
                                       compile_relational_kernel_fact
  ...
  let compiled_step_contracts : step_contract_info list =
    ...
    | None -> []
    | Some contract ->
        let compile_entry_clause (clause : Proof_kernel_types.relational_generated_clause_ir) =
          let has_previous_tick =
            List.exists
              (fun (fact : Proof_kernel_types.relational_clause_fact_ir) ->
                fact.time = Proof_kernel_types.PreviousTick)
        ...
        in
        contract.proof_step_contracts
        |> List.map (fun (proof_step : Proof_kernel_types.proof_step_contract_ir) ->
               let post =
                 proof_step.clauses
                 |> List.filter_map (fun (clause : Proof_kernel_types.relational_generated_clause_ir) ->
                        match clause.origin with
                        | Proof_kernel_types.OriginPropagationNodeInvariant
                        | Proof_kernel_types.OriginPropagationAutomatonCoherence
                        | Proof_kernel_types.OriginSafety ->
  ...
  let labeled_trans =
    List.map
      (fun (t : Why_runtime_view.runtime_transition_view) ->
        let reqs =
          List.map
            (fun (f : Ir.contract_formula) -> (f, origin_label f.origin))
            t.requires
        in
        let ens =
          List.map
  ...
  let post =
    kernel_post_terms @ kernel_step_post_contract_terms
    @ post_contract_terms
  in
  let pre =
