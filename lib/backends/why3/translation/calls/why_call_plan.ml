...
let compile_call_fact_term ~(env : env) ~(summary : Why_runtime_view.callee_summary_view)
    ...
    ~(output_bindings : (Ast.ident * Ast.ident) list) (fact : Proof_kernel_ir.call_fact_ir) :
    Ptree.term option =
  ...
  let lookup =
    ...
    | Proof_kernel_ir.CurrentTick -> current_lookup
    | Proof_kernel_ir.PreviousTick -> previous_lookup
  in
  ...
  | Proof_kernel_ir.FactFormula fo -> Some (compile_call_fo_ltl_term lookup summary fo)
  | Proof_kernel_ir.FactGuaranteeState _ -> None
        ...
        | Some summary ->
            ...
            let delay_output_post_terms =
              ...
              |> List.filter_map (function
                   | Proof_kernel_ir.InstanceDelayCallerPreLink
            ...
            let any_post =
              let tick_posts =
                ...
                | Some tick_summary ->
                    tick_summary.cases
                    |> List.filter_map (fun (case : Proof_kernel_ir.callee_summary_case_ir) ->
                           let premise =
                             case.entry_facts
                             |> List.filter_map
