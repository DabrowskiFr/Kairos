...
let compile_kernel_fact_term ~(env : Support.env) ~(mon_state_ctors : Ast.ident list)
    (fact : Proof_kernel_ir.clause_fact_ir) : Ptree.term option =
  ...
  let compile_desc time = function
    | Proof_kernel_ir.FactProgramState state_name ->
    ...
    | Proof_kernel_ir.FactGuaranteeState _idx ->
        None
    | Proof_kernel_ir.FactFormula fo -> Some (Why_compile_expr.compile_ltl_term_shift env 1 fo)
    | Proof_kernel_ir.FactFalse -> Some (mk_term Tfalse)
  in
...
let compile_node_with_info ?comment_specs ?kernel_ir ~(node_names : Ast.ident list)
  ...
  let phase_case_logic_decls =
    ...
    | None -> []
    | Some (ir : Proof_kernel_ir.node_ir) ->
        ...
        |> List.fold_left
             (fun acc (clause : Proof_kernel_ir.generated_clause_ir) ->
               ...
               | Proof_kernel_ir.OriginSourceProductSummary, ClauseAnchorProductState st -> begin
                   let phase_formula =
                     clause.conclusions
                     |> List.find_map (fun (fact : Proof_kernel_ir.clause_fact_ir) ->
                            match (fact.time, fact.desc) with
                            | Proof_kernel_ir.CurrentTick, Proof_kernel_ir.FactPhaseFormula fo ->
                                Some fo
                            | _ -> None)
                   in
                   ...
                   | Some fo ->
                       add_decl acc
                         (Proof_kernel_ir.phase_state_case_name ~prog_state:st.prog_state
  ...
  let use_kernel_helper_contracts =
    match kernel_ir with
    | Some ir -> Proof_kernel_ir.has_effective_product_coverage ir
    | None -> false
  in
...
let compile_node_from_summary ~prefix_fields ?comment_specs ?kernel_ir
  ...
  compile_node_with_info ?comment_specs ?kernel_ir
    ~node_names:
      (List.map
         (fun (s : Proof_kernel_ir.exported_node_summary_ir) -> s.signature.node_name)
         program_summaries)
    info
