...
let callee_summary_of_exported_summary
    (summary : Proof_kernel_ir.exported_node_summary_ir) : callee_summary_view =
...
let synthetic_transition_of_ir ~(pre_k_updates : Ast.stmt list)
    (t : Proof_kernel_ir.reactive_transition_ir) : Ast.transition =
  {
    Ast.src = t.src_state;
    dst = t.dst_state;
    guard = t.guard_iexpr;
...
let of_exported_summary ?(external_summaries = [])
  ...
  let transitions =
    List.map2
      (fun (tr_ir : Proof_kernel_ir.reactive_transition_ir) t ->
