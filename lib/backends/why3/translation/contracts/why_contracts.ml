...
type compiled_kernel_clause = {
  ...
  anchor_step : Proof_kernel_ir.product_step_ir option;
}
...
let same_runtime_transition_as_step (step : Proof_kernel_ir.product_step_ir)
...
let build_contracts_runtime_view ~(nodes : Ast.node list) ?kernel_ir (info : Why_env.env_info)
  ...
  let kernel_product_coverage =
    ...
    | Some ir -> Proof_kernel_ir.has_effective_product_coverage ir && not !pure_translation
    | None -> false
  in
  ...
  let kernel_clause_origin_label = function
    | Proof_kernel_ir.OriginSourceProductSummary -> "Kernel source product summary"
    | Proof_kernel_ir.OriginPhaseStepPreSummary -> "Kernel phase step pre summary"
    | Proof_kernel_ir.OriginPhaseStepSummary -> "Kernel phase step summary"
    | Proof_kernel_ir.OriginSafety -> "Kernel safety"
    | Proof_kernel_ir.OriginInitNodeInvariant -> "Kernel init node invariant"
    | Proof_kernel_ir.OriginInitAutomatonCoherence -> "Kernel init automaton coherence"
    | Proof_kernel_ir.OriginPropagationNodeInvariant -> "Kernel propagation node invariant"
    | Proof_kernel_ir.OriginPropagationAutomatonCoherence -> "Kernel propagation automaton coherence"
  in
  ...
  let source_summary_formula_for =
    ...
    | Some contract ->
        ...
        let extract_state = function
          | Proof_kernel_ir.
              { time = Proof_kernel_ir.CurrentTick; desc = Proof_kernel_ir.RelFactProgramState st }
            ->
              Some st
          | _ -> None
        in
        let extract_gstate = function
          | Proof_kernel_ir.
              {
                time = Proof_kernel_ir.CurrentTick;
                desc = Proof_kernel_ir.RelFactGuaranteeState idx;
              } ->
              Some idx
          | _ -> None
        in
        let extract_formula = function
          | Proof_kernel_ir.
              { time = Proof_kernel_ir.CurrentTick; desc = Proof_kernel_ir.RelFactFormula fo } ->
              Some fo
          | _ -> None
        in
        List.iter
          (fun (clause : Proof_kernel_ir.relational_generated_clause_ir) ->
            ...
            | Proof_kernel_ir.OriginSourceProductSummary ->
  ...
  let step_phase_formula_for =
    ...
    | Some contract ->
        ...
        let extract_formula = function
          | Proof_kernel_ir.
              { time = Proof_kernel_ir.CurrentTick; desc = Proof_kernel_ir.RelFactPhaseFormula fo }
            ->
              Some fo
          | _ -> None
        in
        List.iter
          (fun (clause : Proof_kernel_ir.relational_generated_clause_ir) ->
            ...
            | Proof_kernel_ir.OriginPhaseStepSummary, ClauseAnchorProductStep step -> begin
  ...
  let step_phase_pre_formula_for =
    ...
    | Some contract ->
        ...
        let extract_formula = function
          | Proof_kernel_ir.
              {
                time = Proof_kernel_ir.PreviousTick;
                desc = Proof_kernel_ir.RelFactPhaseFormula fo;
              } ->
              Some fo
          | _ -> None
        in
        List.iter
          (fun (clause : Proof_kernel_ir.relational_generated_clause_ir) ->
            match (clause.origin, clause.anchor) with
            | Proof_kernel_ir.OriginPhaseStepPreSummary, ClauseAnchorProductStep step -> begin
                match List.find_map extract_formula clause.conclusions with
                | Some fo -> Hashtbl.replace table step fo
                | None -> ()
              end
  ...
  in
  let phase_case_named_term ~(time : Proof_kernel_ir.clause_time_ir) (pred_name : string) :
      Ptree.term =
    match time with
    | Proof_kernel_ir.CurrentTick -> phase_case_pred_term pred_name
    | Proof_kernel_ir.PreviousTick -> term_old (phase_case_pred_term pred_name)
    | Proof_kernel_ir.StepTickContext -> phase_case_pred_term pred_name
  in
  let compile_relational_kernel_fact
      ?anchor_step
      ~(clause_facts : Proof_kernel_ir.relational_clause_fact_ir list)
      (fact : Proof_kernel_ir.relational_clause_fact_ir) : Ptree.term option =
    let compile_desc time = function
      | Proof_kernel_ir.RelFactProgramState state_name ->
          let base = term_eq (term_of_var env "st") (mk_term (Tident (qid1 state_name))) in
          Some
            (match time with
            | Proof_kernel_ir.CurrentTick -> base
            | Proof_kernel_ir.PreviousTick -> term_old base
            | Proof_kernel_ir.StepTickContext -> base)
      | Proof_kernel_ir.RelFactGuaranteeState idx ->
          let paired_prog_state =
            clause_facts
            |> List.find_map (fun (other : Proof_kernel_ir.relational_clause_fact_ir) ->
                   match (other.time, other.desc) with
                   | t, Proof_kernel_ir.RelFactProgramState st when t = time -> Some st
                   | _ -> None)
          in
          ...
          begin
            ...
            | Some prog_state -> begin
                let step_phase_formula =
                  ...
                  | Some (step : Proof_kernel_ir.product_step_ir)
                    when (time = Proof_kernel_ir.CurrentTick
                         || time = Proof_kernel_ir.StepTickContext)
                         && String.equal prog_state step.dst.prog_state
                         && idx = step.dst.guarantee_state_index ->
                      step_phase_formula_for ~step
                  | Some (step : Proof_kernel_ir.product_step_ir)
                    when time = Proof_kernel_ir.PreviousTick
                  ...
                  | Some _ ->
                      Some
                        (phase_case_named_term ~time
                           (Proof_kernel_ir.phase_state_case_name ~prog_state ~guarantee_state:idx))
                  | None -> None
                in
      ...
      | Proof_kernel_ir.RelFactPhaseFormula fo ->
          let base = compile_ltl_term_shift env 1 (normalize_source_summary_fo fo) in
          Some
            (match time with
            | Proof_kernel_ir.CurrentTick -> base
            | Proof_kernel_ir.PreviousTick -> old_if_needed env base
            | Proof_kernel_ir.StepTickContext ->
                compile_ltl_term_shift ~in_post:true env 0 (normalize_source_summary_fo fo))
      | Proof_kernel_ir.RelFactFormula fo ->
          let base = compile_ltl_term_shift env 1 fo in
          Some
            (match time with
            | Proof_kernel_ir.CurrentTick -> base
            | Proof_kernel_ir.PreviousTick -> old_if_needed env base
            | Proof_kernel_ir.StepTickContext -> compile_ltl_term_shift ~in_post:true env 0 fo)
      | Proof_kernel_ir.RelFactFalse -> Some (mk_term Tfalse)
    in
    compile_desc fact.time fact.desc
  in
  let compile_relational_kernel_clause_summary
      ~(idx : int) ~(label : string) (clause : Proof_kernel_ir.relational_generated_clause_ir) :
      compiled_kernel_clause option =
    let anchor_step =
      match clause.anchor with
      | Proof_kernel_ir.ClauseAnchorProductStep step -> Some step
      | Proof_kernel_ir.ClauseAnchorProductState _ -> None
    in
    let clause_source_state =
      clause.hypotheses
      |> List.find_map (fun (fact : Proof_kernel_ir.relational_clause_fact_ir) ->
             match (fact.time, fact.desc) with
             | Proof_kernel_ir.PreviousTick, Proof_kernel_ir.RelFactProgramState state_name ->
                 Some state_name
             | Proof_kernel_ir.CurrentTick, Proof_kernel_ir.RelFactProgramState state_name ->
                 Some state_name
             | _ -> None)
    in
    let premise =
  ...
  let compile_merged_relational_kernel_clause_summary ~(idx : int) ~(label : string)
      (clauses : Proof_kernel_ir.relational_generated_clause_ir list) :
      compiled_kernel_clause option =
    match clauses with
    | [] -> None
    | first_clause :: _ ->
        ...
        let conclusion =
          clauses
          |> List.concat_map
               (fun (clause : Proof_kernel_ir.relational_generated_clause_ir) -> clause.conclusions)
          |> List.filter_map
        ...
        Option.map
          ...
          conclusion
  in
  let same_rel_step_clause_shape (a : Proof_kernel_ir.relational_generated_clause_ir)
      (b : Proof_kernel_ir.relational_generated_clause_ir) : bool =
    ...
    | Some contract ->
        if contract.obligations.symbolic = [] && contract.proof_step_contracts = [] then
          ...
          let propagation_safety_and_phase =
            ...
            |> List.concat_map (fun (proof_step : Proof_kernel_ir.proof_step_contract_ir) ->
                   proof_step.clauses)
          in
          let rec consume_rel acc = function
            | [] -> acc
            | (idx_node, (node_clause : Proof_kernel_ir.relational_generated_clause_ir))
              :: (_, (safety_clause : Proof_kernel_ir.relational_generated_clause_ir))
              :: rest
              when node_clause.origin = Proof_kernel_ir.OriginPropagationNodeInvariant
                   && safety_clause.origin = Proof_kernel_ir.OriginSafety
                   && same_rel_step_clause_shape node_clause safety_clause ->
                let acc =
                  match
                    compile_relational_kernel_clause_summary ~idx:idx_node
                      ~label:(kernel_clause_origin_label Proof_kernel_ir.OriginSafety)
                      safety_clause
                  with
                  | None -> acc
                  | Some clause ->
            ...
            | (idx_node, (node_clause : Proof_kernel_ir.relational_generated_clause_ir)) :: rest
              when node_clause.origin = Proof_kernel_ir.OriginPropagationNodeInvariant ->
                let acc =
                ...
                in
                consume_rel acc rest
            | (idx, (clause : Proof_kernel_ir.relational_generated_clause_ir)) :: rest ->
                let compiled =
                  match clause.origin with
                  | Proof_kernel_ir.OriginPropagationNodeInvariant
                  | Proof_kernel_ir.OriginPropagationAutomatonCoherence
                  | Proof_kernel_ir.OriginSafety ->
                      compile_relational_kernel_clause_summary ~idx
                        ~label:(kernel_clause_origin_label clause.origin)
                        clause
                  | Proof_kernel_ir.OriginSourceProductSummary
                  | Proof_kernel_ir.OriginPhaseStepPreSummary
                  | Proof_kernel_ir.OriginPhaseStepSummary
                  | Proof_kernel_ir.OriginInitNodeInvariant
                  | Proof_kernel_ir.OriginInitAutomatonCoherence -> None
                in
                let acc =
                  match compiled with
                  | None -> acc
    ...
    | Some contract ->
        let from_step_entry_clauses =
          if contract.obligations.symbolic = [] && contract.proof_step_contracts = [] then
            ...
            contract.proof_step_contracts
            |> List.fold_left
                 (fun (terms, labeled, states) (proof_step : Proof_kernel_ir.proof_step_contract_ir) ->
                   proof_step.entry_clauses
                   |> List.fold_left
                        (fun (terms, labeled, states)
                             (clause : Proof_kernel_ir.relational_generated_clause_ir) ->
                          let premise =
                            clause.hypotheses
                            |> List.filter_map
                          ...
                          let conclusion =
                            clause.conclusions
                            |> List.filter_map (fun (fact : Proof_kernel_ir.relational_clause_fact_ir) ->
                                   ...
                                   | Proof_kernel_ir.RelFactFormula fo ->
                                       ...
                                       let fact =
                                         {
                                           fact with
                                           desc = Proof_kernel_ir.RelFactFormula normalized;
                                         }
                                       in
                                       compile_relational_kernel_fact
                                         ~clause_facts:clause.conclusions fact
                                   | Proof_kernel_ir.RelFactPhaseFormula _ ->
                                       compile_relational_kernel_fact
                                         ~clause_facts:clause.conclusions fact
                                   | _ ->
                                       compile_relational_kernel_fact
  ...
  let compiled_step_contracts : step_contract_info list =
    ...
    | None -> []
    | Some contract ->
        let compile_entry_clause (clause : Proof_kernel_ir.relational_generated_clause_ir) =
          let has_previous_tick =
            List.exists
              (fun (fact : Proof_kernel_ir.relational_clause_fact_ir) ->
                fact.time = Proof_kernel_ir.PreviousTick)
        ...
        in
        contract.proof_step_contracts
        |> List.map (fun (proof_step : Proof_kernel_ir.proof_step_contract_ir) ->
               let post =
                 proof_step.clauses
                 |> List.filter_map (fun (clause : Proof_kernel_ir.relational_generated_clause_ir) ->
                        match clause.origin with
                        | Proof_kernel_ir.OriginPropagationNodeInvariant
                        | Proof_kernel_ir.OriginPropagationAutomatonCoherence
                        | Proof_kernel_ir.OriginSafety ->
