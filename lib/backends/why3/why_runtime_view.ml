...
let callee_summary_of_exported_summary
    (summary : Proof_kernel_types.exported_node_summary_ir) : callee_summary_view =
         ...
         (fun acc (_, info) ->
           if List.exists
                (fun (existing : Temporal_support.pre_k_info) ->
                  existing.Temporal_support.expr = info.Temporal_support.expr
...
let synthetic_transition_of_ir ~(pre_k_updates : Ast.stmt list)
    (t : Proof_kernel_types.reactive_transition_ir) : Ast.transition =
  {
    Ast.src = t.src_state;
    dst = t.dst_state;
    guard = t.guard_iexpr;
...
(** Convert a [verified_transition] to an [Ast.transition] suitable for
    ...
    the IR path. *)
let ast_of_verified_transition (t : Proof_obligation_ir.verified_transition) : Ast.transition =
  {
    Ast.src = t.src_state;
    dst = t.dst_state;
    guard = t.guard_iexpr;
...
let ast_node_of_verified_node (vn : Proof_obligation_ir.verified_node) : Ast.node =
  {
    semantics =
      {
...
let of_exported_summary ~(program_summaries : Proof_kernel_types.exported_node_summary_ir list)
    (summary : Proof_kernel_types.exported_node_summary_ir) : t =
  ...
  let local_summaries =
    program_summaries
    |> List.filter_map (fun (s : Proof_kernel_types.exported_node_summary_ir) ->
  ...
  let transitions =
    List.map2
      (fun (tr_ir : Proof_kernel_types.reactive_transition_ir) t ->
