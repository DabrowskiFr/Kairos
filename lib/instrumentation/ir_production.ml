...
let raw_transition_of_abs (t : Abs.transition) : Kairos_ir.raw_transition =
  {
    Kairos_ir.src_state             = t.src;
    dst_state                       = t.dst;
...
let build_raw_node (node : Abs.node) : Kairos_ir.raw_node =
  ...
  {
    Kairos_ir.node_name      = node.semantics.sem_nname;
