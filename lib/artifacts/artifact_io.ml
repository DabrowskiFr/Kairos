...
let emit_dot_files ~(show_labels : bool) ~(out_file : string) (program : Ast.program) : unit =
  let dot, labels = Artifact_render_monitor.dot_monitor_program ~show_labels program in
  if out_file = "-" then (
    print_string dot;
...
let emit_why3_vc ~(out_file : string) ~(why_text : string) : unit =
  let tasks = Why_contract_prove.dump_why3_tasks ~text:why_text in
...
let emit_smt2 ~(out_file : string) ~(prover : string) ~(why_text : string) : unit =
  let tasks = Why_contract_prove.dump_smt2_tasks ~prover ~text:why_text in
  let buf = Buffer.create 4096 in
  List.iteri
    (fun i task ->
      if i > 0 then Buffer.add_string buf "\n; ---- goal ----\n";
...
let prove_why ~(prover : string) ~(prover_cmd : string option) ~(why_text : string) : unit =
  ...
  let result = Why_contract_prove.prove_text ~prover ?prover_cmd ~text:why_text () in
