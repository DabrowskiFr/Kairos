...
(** Generation of Why3 contracts from product-step clauses.
    ...
    contracts for helper functions generated by {!Why_compile}. The global
    labels are kept only for Why3 text/proof-task presentation. *)
open Core_syntax
...
type contract_info = {
  pre_labels : string list;
      (** Short labels associated with generated preconditions. *)
  post_labels : string list;
      (** Short labels associated with generated postconditions. *)
  step_contracts : step_contract_info list;
      (** Per-step contracts for the product. *)
}

(** [build_contracts ~env ~runtime ~pure_translation] generates product-step
    contracts for the node described by [runtime].
