...
let rec term_has_old (term : Ptree.term) : bool =
  ...
  | Tapply (fn, arg) -> (
      ...
      | Tident qid when String.equal (Why_compile_expr.string_of_qid qid) "old"
        ->
          true
  ...
  | Tupdate (base, fields) ->
      term_has_old base
      || List.exists (fun (_field, term) -> term_has_old term) fields
  | Ttrue | Tfalse | Tconst _ | Tident _ | Tasref _ -> false

let names_of_spec (spc : Ptree.spec) (acc : StringSet.t) : StringSet.t =
  let acc =
    List.fold_left (fun acc term -> names_of_term term acc) acc spc.sp_pre
  in
  let acc =
    List.fold_left
      (fun acc (_loc, posts) ->
  ...
  in
  let acc =
    List.fold_left (fun acc term -> names_of_term term acc) acc spc.sp_writes
  in
  let acc =
    List.fold_left
...
let rec names_of_expr (expr : Ptree.expr) (acc : StringSet.t) : StringSet.t =
  ...
  | Ewhile (cond, invariant, variant, body) ->
      let acc =
        List.fold_left (fun acc term -> names_of_term term acc) acc invariant
      in
      names_of_expr body (names_of_variant variant (names_of_expr cond acc))
  | Enot inner | Ecast (inner, _) | Eghost inner | Eattr (_, inner)
  | Elabel (_, inner) | Escope (_, inner) ->
