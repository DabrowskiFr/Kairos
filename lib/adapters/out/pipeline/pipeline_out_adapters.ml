...
module Why_text = struct
  let why_text ~(snapshot : Pipeline_types.pipeline_snapshot) : Pipeline_types.why_outputs =
    ...
    { Pipeline_types.why_text; stage_meta = Pipeline_outputs.stage_meta snapshot.infos }
end

module Obligations = struct
  let obligations ~(snapshot : Pipeline_types.pipeline_snapshot) : Pipeline_types.obligations_outputs =
...
module Ir_render = struct
  let normalized_program ~(snapshot : Pipeline_types.pipeline_snapshot) : string =
  ...
  let pretty_program ~(snapshot : Pipeline_types.pipeline_snapshot) : string =
...
module Proof_events = struct
  let prove_with_events ~timeout_s ~should_cancel ~(snapshot : Pipeline_types.pipeline_snapshot)
