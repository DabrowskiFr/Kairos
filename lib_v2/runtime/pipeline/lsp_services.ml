...
let diagnostics_for_text ~uri:_ ~(text : string) : diagnostic list =
  ...
  let res =
    try
      match Pipeline.build_ast_with_info ~input_file:tmp () with
      | Ok ((_asts, infos)) ->
          let diags = ref [] in
      ...
      | Error (Pipeline.Parse_error msg) ->
          [ mk_diag ~severity:1 ~source:"kairos-parse" ~message:msg ]
      | Error e ->
          [ mk_diag ~severity:1 ~source:"kairos-pipeline" ~message:(Pipeline.error_to_string e) ]
    with exn ->
