...
let program_stats (p:Ast.program) : (string * string) list =
  ...
  let transitions =
    List.fold_left (fun acc (n:Ast.node) -> acc + List.length (n.trans)) 0 p
  in
  let requires =
    List.fold_left
      (fun acc (n:Ast.node) ->
  ...
  in
  let locals =
    List.fold_left (fun acc n -> acc + List.length (n.locals)) 0 p
  in
...
let stage_meta (infos:stage_infos) : (string * (string * string) list) list =
  let parse =
    Option.value ~default:Stage_info.empty_parse_info infos.parse
  in
  let automaton_info =
    Option.value ~default:Stage_info.empty_automaton_info infos.automaton
  in
  let contracts_info =
    Option.value ~default:Stage_info.empty_contracts_info infos.contracts
  in
  let monitor_info =
    Option.value ~default:Stage_info.empty_monitor_info infos.monitor
  in
  let obc_info =
    Option.value ~default:Stage_info.empty_obc_info infos.obc
  in
  let user =
    ( "Parse",
  ...
  let automaton =
    ( "Automaton",
      [ ("states", string_of_int automaton_info.residual_state_count);
        ("edges", string_of_int automaton_info.residual_edge_count);
        ("warnings", string_of_int (List.length automaton_info.warnings)) ] )
  in
  let contracts =
    ( "Contracts",
...
let emit_automaton_debug_stats (p:Ast.program) =
  let nodes = List.length p in
  let edges =
    List.fold_left (fun acc n -> acc + List.length (n.trans)) 0 p
  in
  Log.stage_info
...
let build_ast_with_info ?(log=false) ~input_file ()
  : (ast_stages * stage_infos, error) result =
  let parse () =
    try
