...
let instrumentation_pass oc ~id ~params =
  match Lsp_pipeline_pass_decode.instrumentation_pass params with
  | Some req when Lsp_pipeline_pass_input.valid_file req.input_file -> (
      match Lsp_backend_usecases.instrumentation_pass req with
      | Ok out ->
