        ...
        | Ok ir ->
            List.iter
              (fun (raw : Proof_obligation_ir.raw_node) ->
                ...
                write_file dir (name ^ ".raw.kir") (Artifact_render_ir.render_raw_node raw))
              ir.raw_ir_nodes;
            List.iter
              (fun (ann : Proof_obligation_ir.annotated_node) ->
                let name = ann.raw.node_name in
                write_file dir (name ^ ".annotated.kir")
                  (Artifact_render_ir.render_annotated_node ann);
                write_file dir (name ^ ".annotated.dot")
                  (Artifact_render_ir_dot.dot_of_annotated_node ann))
              ir.annotated_ir_nodes;
            List.iter
              (fun (ver : Proof_obligation_ir.verified_node) ->
                let name = ver.node_name in
                write_file dir (name ^ ".verified.kir")
                  (Artifact_render_ir.render_verified_node ver);
...
let cmd =
  ...
  in
  Cmd.v
    (Cmd.info "kairos-pipeline" ~version:"0.1"
       ~doc:"Kairos architecture-driven pipeline CLI")
    term
