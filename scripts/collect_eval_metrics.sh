...
Options:
  ...
  --trace-level LEVEL  Traceability level for Kairos (full|minimal|off, default: full)
...
case "$TRACE_LEVEL" in
  full|minimal|off) ;;
  *)
    echo "Invalid --trace-level: $TRACE_LEVEL (expected: full|minimal|off)" >&2
    exit 2
    ;;
esac
...
TRACE_ARGS=(--trace-level "$TRACE_LEVEL")
...
for entry in "${EXAMPLES[@]}"; do
  ...
  "$CLI" "${TRACE_ARGS[@]}" --dump-product "$PRODUCT_TXT" "$FILE"
  "$CLI" "${TRACE_ARGS[@]}" --dump-obligations-map "$OBLIGATIONS_TXT" "$FILE"
  "$CLI" "${TRACE_ARGS[@]}" --dump-why3-vc "$VC_TXT" "$FILE"

  set +e
  /usr/bin/time -p "$CLI" "${TRACE_ARGS[@]}" --prove --timeout-s "$TIMEOUT_S" --dump-timings "$TIMINGS_CSV" "$FILE" > "$PROVE_OUT" 2> "$PROVE_ERR"
  PROVE_CODE=$?
  set -e
  ...
  if [[ "$CANONICAL_CONTRACTS" == "NA" || "$SAFE_CASES" == "NA" || "$BAD_G_CASES" == "NA" ]]; then
    ...
    "$CLI" "${TRACE_ARGS[@]}" --dump-canonical "$CANONICAL_DOT" "$FILE"
