...
let string_of_spot_ltl ~(atom_names : string list) (formula : Automata_exchange.ltl) : string =
  let atom_name atom =
    let rec find index = function
      | [] -> invalid_arg ("Spot adapter: undeclared atom " ^ atom)
      | candidate :: rest ->
          if String.equal candidate atom then spot_ap_name index else find (index + 1) rest
    in
    find 0 atom_names
  in
  let rec go ~(ctx : int) = function
    | Automata_exchange.True -> "1"
    | False -> "0"
...
let ensure_safety ~record_elapsed (formula : string) : unit =
...
let parse_ap_count (line : string) : int =
  ...
  let body = match String.index_opt body ' ' with Some i -> String.sub body 0 i | None -> body in
  parse_int body
...
let parse_transition_line (line : string) : string * int =
  let rb =
    ...
    | Some i -> i
    | None -> failwith ("Invalid HOA label: " ^ line)
  in
  let label = String.sub line 1 (rb - 1) in
  let dest = String.sub line (rb + 1) (String.length line - rb - 1) |> String.trim in
  let dest = match String.index_opt dest ' ' with Some i -> String.sub dest 0 i | None -> dest in
...
let parse_label (text : string) : label_expr =
  ...
  and parse_atom i =
    ...
    if i >= len then failwith ("Unexpected end of HOA label: " ^ text)
      ...
      | '0' .. '9' ->
          let j = ref (i + 1) in
          while !j < len && match text.[!j] with '0' .. '9' -> true | _ -> false do
            incr j
          done;
  ...
  let rec loop start ap_count ap_names acceptance states current in_body = function
    ...
    | line :: rest when in_body && starts_with ~prefix:"State:" line ->
        ...
        let id, accepting =
          ...
          match acceptance with Some Acceptance_all -> (id, true) | _ -> (id, accepting)
        in
...
let rec label_to_dnf = function
  ...
  | Label_and (a, b) ->
      ...
      List.concat_map (fun ta -> List.filter_map (fun tb -> Some (ta @ tb)) db) da
...
let normalize_cube (cube : (int * bool) list) : (int * bool) list option =
  ...
  List.iter
    ...
    cube;
  if not !ok then None
  else Some (Hashtbl.fold (fun idx value acc -> (idx, value) :: acc) tbl [] |> List.sort compare)
...
let raw_guard_of_label ~(atom_names : string list) ~(hoa_ap_names : string list)
    (label : label_expr) : raw_guard =
  let hoa_to_kairos_idx =
    if hoa_ap_names = [] then fun i -> i
    else fun hoa_i ->
  ...
  let term_of_cube (cube : (int * bool) list) : term =
    ...
    List.mapi
      ...
      atom_names
  in
  label_to_dnf label |> List.filter_map normalize_cube |> List.map term_of_cube
