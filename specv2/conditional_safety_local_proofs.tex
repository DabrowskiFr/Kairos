...
truth of the user invariants on executions, implies conditional safety. We
also prove two relative-completeness results for the reduction itself: global
correctness implies validity of all generated safety triples, and, under true
user invariants, validity of all generated triples. The
...
semantic truth of the user invariants on executions, implies conditional
safety. Second, global correctness implies validity of the generated
$\NoBad$ triples. Third, under the additional assumption that user invariants
are semantically true on executions, all generated triples are valid. The
first theorem justifies the reduction as a sound proof method; the second and
third explain why the reduction generates semantically legitimate local proof
objects rather than arbitrary artifacts.
...
invariants on executions. Concrete choices such as formula syntax, encoding
...
validity and on the semantic truth of user invariants. Finally, it provides a
mechanized Rocq development whose public structure mirrors the mathematical
argument.
...
\begin{tikzpicture}[
  ...
  autA/.style={draw,circle,minimum size=8mm,inner sep=1pt,fill=assumptionbg,draw=assumptionc!80!black},
  autG/.style={draw,circle,minimum size=8mm,inner sep=1pt,fill=guaranteebg,draw=guaranteec!75!black},
  bad/.style={draw,circle,minimum size=8mm,inner sep=1pt,fill=dangerbg,draw=dangerc},
  arr/.style={-Latex,thick},
  lab/.style={fill=white,inner sep=1pt,font=\scriptsize,align=center}
]
  \node[align=center,font=\small\bfseries,text=assumptionc!80!black] at (1.5,1.6) {Assumption automaton $A$};
  \node[autA] (a0) at (0,0) {$A_0$};
  \node[autA] (a1) at (3.2,0) {$A_1$};
  \node[bad] (abad) at (6.2,0) {$A_{\mathsf{bad}}$};
  ...
  \node[align=center,font=\small\bfseries,text=guaranteec!75!black] at (2.8,-2.0) {Guarantee automaton $G$};
  \node[autG] (g0) at (0,-3.5) {$G_0$};
  \node[autG] (g1) at (2.7,-3.5) {$G_1$};
  \node[autG] (g2) at (5.4,-3.5) {$G_2$};
  \node[bad] (gbad) at (8.6,-3.5) {$G_{\mathsf{bad}}$};
...
\begin{tikzpicture}[
  every node/.style={font=\small},
  pstate/.style={draw,rounded corners,minimum width=3.2cm,minimum height=1cm,align=center,fill=coherencebg,draw=black!55},
  badstate/.style={draw,rounded corners,minimum width=3.4cm,minimum height=1cm,align=center,fill=dangerbg,draw=dangerc},
  fact/.style={draw,rounded corners,minimum width=3.3cm,minimum height=0.95cm,align=center,fill=coherencebg,draw=assumptionc!45!black},
  clause/.style={draw,rounded corners,minimum width=3.8cm,minimum height=0.95cm,align=center,fill=clausebg,draw=guaranteec!45!black},
  darr/.style={-Latex,thick,draw=dangerc}
]
\node[pstate] (src) at (0,0.6) {$p_{\mathrm{src}}$\\$(\mathsf{Run},q_A,q_G)$};
\node[badstate] (dst) at (6.0,0.6) {$p_{\mathrm{dst}}$\\$(\mathsf{Run},q_A,q^G_{\mathsf{bad}})$};
\draw[darr] (src) -- node[above,align=center,fill=white,inner sep=1pt]
  {$r=0$ and $y \neq \mathsf{prev}(x)$} (dst);
\node[fact] (pre) at (0,-1.5) {\textcolor{assumptionc!80!black}{precondition facts}\\$\CohCur_{p_{\mathrm{src}}} \land \NodeInv_{\mathsf{Run}}$};
\node[clause] (post) at (6.0,-1.5) {\textcolor{guaranteec!75!black}{generated safety objects}\\$\Bad(p)$ and $\NoBad(p)$};
\end{tikzpicture}
...
5. Valid generated triples, together with true user invariants, imply conditional safety. \\
\end{tabular}
\end{center}
...
triples themselves is the truth of user invariants on executions; this
...
\begin{tikzpicture}[
  ...
  triplebox/.style={draw=guaranteec!55!black, rounded corners=4pt, fill=guaranteebg,
    minimum width=2.95cm, minimum height=0.95cm, align=center},
  arr/.style={-Latex, thick}
]
\node[initbox] (init) {initial tick\\$(\mathsf{Init},A_0,G_0)$};
\node[factbox, right=1.9cm of init, yshift=0.7cm] (initinv) {$\NodeInv_{\mathsf{Init}}$};
\node[factbox, right=1.9cm of init, yshift=-0.7cm] (initcoh) {$\CohCur_{(\mathsf{Init},A_0,G_0)}$};
\node[triplebox, right=1.9cm of initinv] (tinitinv) {$\tripleinv{h^{\mathsf{init}}_{\NodeInv}}$};
\node[triplebox, right=1.9cm of initcoh] (tinitcoh) {$\triplecoh{h^{\mathsf{init}}_{\Coherence}}$};
\draw[arr] (init) -- (initinv);
\draw[arr] (init) -- (initcoh);
\draw[arr] (initinv) -- (tinitinv);
\draw[arr] (initcoh) -- (tinitcoh);

\node[propbox, below=2.0cm of init, xshift=-1.7cm] (src) {$p_{\mathrm{src}}$\\$(s,q_A,q_G)$};
\node[propbox, right=2.4cm of src] (step) {product step $p$};
\node[propbox, right=2.4cm of step] (dst) {$p_{\mathrm{dst}}$\\$(s',q'_A,q'_G)$};
\draw[arr] (src) -- (step);
\draw[arr] (step) -- (dst);

\node[factbox, below=1.45cm of src, xshift=0.9cm] (srcfacts) {$\NodeInv_s \wedge \CohCur_{p_{\mathrm{src}}}$};
\node[factbox, below=1.45cm of dst, xshift=-0.9cm] (dstfacts) {$\NodeInv_{s'} \wedge \CohCur_{p_{\mathrm{dst}}}$};
\node[triplebox, below=1.45cm of step, yshift=0.55cm] (tpropinv) {$\tripleinv{h^{\mathsf{prop}}_{\NodeInv}(p)}$};
\node[triplebox, below=1.45cm of step, yshift=-0.75cm] (tpropcoh) {$\triplecoh{h^{\mathsf{prop}}_{\Coherence}(p)}$};
...
triples, together with semantic truth of the user invariants, implies
...
of the generated triples and of user invariants on executions when proving
soundness, and only the truth of user invariants when proving full relative
completeness. No statement in this
...
supplied with the specification, so their truth on executions cannot be
...
the safety triple $\triplesafe{h^{\NoBad}(p)}$ is indeed one of the generated triples.
\end{proof}

\subsection{Soundness}
The first result justifies the reduction. Automaton coherence is reconstructed from the product semantics itself, whereas user invariants belong to the specification and are therefore assumed semantically true on executions through $\InvTrue$.
...
By $\InvTrue$, the corresponding user-invariant clause also holds at tick $k$:
\[
  \NodeInv_{\pi_S(p_{\mathrm{src}})}(\Ctx_{m_0,u}(k)).
\]
...
user invariants are indeed true on executions. The resulting notion is weaker
...
semantically valid under the natural hypotheses of the model.
...
If $h = \tripleinv{h^{\mathsf{init}}_{\NodeInv}}$, validity follows immediately from
$\InvTrue$ at tick $0$.

If $h = \triplecoh{h^{\mathsf{init}}_{\Coherence}}$, validity is immediate from the
...
true user invariants, are sufficient for global conditional safety. Relative
completeness states that, under the appropriate semantic hypotheses, the
reduction does not generate arbitrary false local proof objects. This is the
