...
\lstdefinestyle{kairosstyle}{
  ...
  aboveskip=0.6em,
  belowskip=0.4em,
  emph={requires,ensures,invariants,transitions},
  emphstyle=\color{guaranteec}\bfseries
}
...
\AtBeginDocument{%
  \fancypagestyle{standardpagestyle}{%
    ...
    \fancyfoot[RO,LE]{\footnotesize Preprint}%
  }%
...
reduction is sound: validity of all generated triples, together with semantic
truth of the user invariants on admissible executions, implies conditional
safety. We also prove two relative-completeness results for the reduction
itself: global correctness implies admissible-run validity of all generated
safety triples, and, under true user invariants on admissible executions,
admissible-run validity of all generated triples. The
...
<ccs2012>
<concept>
<concept_id>10011007.10011006.10011039.10011688</concept_id>
<concept_desc>Software and its engineering~Formal methods</concept_desc>
<concept_significance>500</concept_significance>
</concept>
<concept>
<concept_id>10011007.10011006.10011039.10011693</concept_id>
...
The non-trivial point is therefore not the mere existence of safety automata,
nor the observation that local verification conditions are useful in practice.
The non-trivial point is to identify the semantic object that makes the local
proof layer both inevitable and backend-independent. The thesis of this paper
is that the explicit product between the program and the two safety automata is
...
results. First, validity of all generated relational triples, together with
semantic truth of the user invariants on admissible executions, implies
conditional safety. Second, global correctness implies admissible-run validity
of the generated $\NoBad$ triples. Third, under the additional assumption that
user invariants are semantically true on admissible executions, all generated
triples are admissibly valid. The first theorem justifies the reduction as a
sound proof method; the second and third explain why the reduction generates
semantically legitimate local proof objects rather than arbitrary artifacts.
...
product semantics, together with the semantic truth of user-supplied
invariants on admissible executions. Concrete choices such as formula syntax, encoding
into a verification-condition language, proof search, or solver orchestration
belong to later instantiation layers. This separation is deliberate: it keeps
the semantic theorem stable while allowing multiple backends to refine it.
...
reduction, making explicit that soundness relies only on generated-triple
validity and on the semantic truth of user invariants on admissible
executions. Finally, it provides a mechanized Rocq development whose public
...
For readability, the figures below abbreviate the main labels of the running
example as follows:
\[
\varphi_A \triangleq (r \in \{0,1\}) \land (r = 1 \Rightarrow x = 0),
\]
\[
\psi_{\mathit{rst}} \triangleq (r = 1 \Rightarrow y = 0),\qquad
\psi_{\mathit{first}} \triangleq (r = 0 \land \pre(r)=1 \Rightarrow y = 0),
\]
\[
\psi_{\mathit{delay}} \triangleq
(r = 0 \land \pre(r)=0 \Rightarrow y = \pre(x)).
\]
...
\begin{tikzpicture}[
  ...
  autA/.style={draw,circle,minimum size=7.2mm,inner sep=0.8pt,fill=assumptionbg,draw=assumptionc!80!black},
  autG/.style={draw,circle,minimum size=7.2mm,inner sep=0.8pt,fill=guaranteebg,draw=guaranteec!75!black},
  bad/.style={draw,circle,minimum size=7.2mm,inner sep=0.8pt,fill=dangerbg,draw=dangerc},
  arr/.style={-Latex,thick},
  lab/.style={fill=white,inner sep=1pt,font=\scriptsize,align=center}
]
  \node[align=center,font=\small\bfseries,text=assumptionc!80!black] at (1.3,1.45) {Assumption automaton $A$};
  \node[autA] (a0) at (0,0) {$A_0$};
  \node[autA] (a1) at (2.7,0) {$A_1$};
  \node[bad] (abad) at (5.1,0) {$A_{\mathsf{bad}}$};
  \draw[arr,draw=assumptionc!85!black] (a0) -- node[lab,above] {$\varphi_A$} (a1);
  \draw[arr,draw=assumptionc!85!black] (a1) edge[loop above] node[lab] {$\varphi_A$} (a1);
  \draw[arr,draw=dangerc] (a0) to[bend left=16] node[lab,below] {otherwise} (abad);
  \draw[arr,draw=dangerc] (a1) to[bend left=12] node[lab,above] {otherwise} (abad);

  \node[align=center,font=\small\bfseries,text=guaranteec!75!black] at (2.3,-1.8) {Guarantee automaton $G$};
  \node[autG] (g0) at (0,-3.1) {$G_0$};
  \node[autG] (g1) at (2.3,-3.1) {$G_1$};
  \node[autG] (g2) at (4.6,-3.1) {$G_2$};
  \node[bad] (gbad) at (7.2,-3.1) {$G_{\mathsf{bad}}$};
  \draw[arr,draw=guaranteec!80!black] (g0) -- node[lab,above] {$\psi_{\mathit{rst}}$} (g1);
  \draw[arr,draw=guaranteec!80!black] (g1) -- node[lab,above] {$\psi_{\mathit{first}}$} (g2);
  \draw[arr,draw=guaranteec!80!black] (g2) edge[loop below] node[lab] {$\psi_{\mathit{delay}}$} (g2);
  \draw[arr,draw=dangerc] (g0) to[bend right=14] node[lab,below] {otherwise} (gbad);
  \draw[arr,draw=dangerc] (g1) to[bend right=14] node[lab,below] {otherwise} (gbad);
  \draw[arr,draw=dangerc] (g2) to[bend right=10] node[lab,below] {otherwise} (gbad);
\end{tikzpicture}}
\caption{Safety automata for the running example. The assumption automaton
tracks the reset discipline, while the guarantee automaton distinguishes three
residual modes: reset ticks ($G_0$), the first non-reset tick after reset
($G_1$), and the ordinary one-step delay case ($G_2$). In both automata, the
red ``otherwise'' transitions collect the locally unsafe cases and lead to the
bad state.}
...
the active guarantee branch is $G_2$ and the expected output is
...
\begin{tikzpicture}[
  every node/.style={font=\small},
  pstate/.style={draw,rounded corners,minimum width=2.85cm,minimum height=0.9cm,align=center,fill=coherencebg,draw=black!55},
  badstate/.style={draw,rounded corners,minimum width=3.0cm,minimum height=0.9cm,align=center,fill=dangerbg,draw=dangerc},
  fact/.style={draw,rounded corners,minimum width=2.95cm,minimum height=0.85cm,align=center,fill=coherencebg,draw=assumptionc!45!black},
  clause/.style={draw,rounded corners,minimum width=3.35cm,minimum height=0.85cm,align=center,fill=clausebg,draw=guaranteec!45!black},
  darr/.style={-Latex,thick,draw=dangerc}
]
\node[pstate] (src) at (0,0.6) {$p_{\mathrm{src}}$\\$(\mathsf{Run},q_A,q_G)$};
\node[badstate] (dst) at (5.2,0.6) {$p_{\mathrm{dst}}$\\$(\mathsf{Run},q_A,q^G_{\mathsf{bad}})$};
\draw[darr] (src) -- node[above,align=center,fill=white,inner sep=1pt]
  {$r=0$ and $y \neq \mathsf{prev}(x)$} (dst);
\node[fact] (pre) at (0,-1.35) {\textcolor{assumptionc!80!black}{precondition facts}\\$\CohCur_{p_{\mathrm{src}}} \land \NodeInv_{\mathsf{Run}}$};
\node[clause] (post) at (5.2,-1.35) {\textcolor{guaranteec!75!black}{generated safety objects}\\$\Bad(p)$ and $\NoBad(p)$};
\end{tikzpicture}}
\caption{Relevant fragment of the explicit product for the running example.
The source state combines the current control state with the current assumption
and guarantee automaton states. The red edge is dangerous because its
assumption component remains admissible while its guarantee component reaches
the bad state. If a concrete tick matches this edge, the generated triple
$\triplesafe{h^{\NoBad}(p)}$ is activated.}
...
Figure~\ref{fig:running-automata} explains which abstract modes of the
assumption and guarantee are relevant on the running example. The labels
$\varphi_A$, $\psi_{\mathit{rst}}$, $\psi_{\mathit{first}}$, and
$\psi_{\mathit{delay}}$ are exactly the local predicates that distinguish the
reset case, the first non-reset tick after reset, and the ordinary delay case.
Figure~\ref{fig:dangerous-fragment} then isolates the single dangerous product
edge used throughout the proof sketch. Here $q_A$ and $q_G$ denote the current
states of the assumption and guarantee automata in the source product state.
The facts $\CohCur_{p_{\mathrm{src}}}$ and $\NodeInv_{\mathsf{Run}}$ justify
the precondition of the local safety triple; $\Bad(p)$ classifies the edge as
dangerous, whereas $\NoBad(p)$ is the semantic clause generated from this
classification and embedded into the triple
$\triplesafe{h^{\NoBad}(p)}$.
...
3. The clause is packaged into one local safety triple $\triplesafe{h^{\NoBad}(p)}$. \\
4. Coherence triples justify the local preconditions of $\triplesafe{h^{\NoBad}(p)}$. \\
5. Valid generated triples, together with true user invariants on admissible executions, imply conditional safety. \\
\end{tabular}
\end{center}
...
definitions alone. The only semantic fact not reconstructed from generated
triples themselves is the truth of user invariants on admissible executions; this
dependency will be isolated explicitly in the meta-theory.
...
The transition guard is represented by $\mathsf{enabled}$. Since
$\mathsf{select}(s,m,i)$ is required to return an enabled transition, every
concrete tick already carries the fact that the guard of the selected
transition holds. Later triples therefore do not need to repeat this guard as
an extra ad hoc premise: it is already built into the semantic notion of a
realized step.
...
\noindent\leadcolor{Running example.}
...
Matching also records the selected program transition itself. As a consequence,
once $\Match(\Ctx_{m_0,u}(k),p)$ holds, the guard of the transition carried by
$p$ is already known to hold at the current tick, because the matched
transition is exactly the one returned by $\mathsf{select}$.
...
A semantic clause is a predicate over tick contexts
\[
  C : \Ctx \to \mathsf{Prop}.
\]
\noindent where $\Ctx$ denotes the ambient space of tick contexts.
\end{definition}
...
\begin{definition}[Admissible-run triple validity]
\[
  \begin{aligned}
    \ValidAdm((\mathit{init},\pre,\post))
    \triangleq{}& \forall m_0 \in M,\forall u : \mathbb{N} \to I.\\
    &\bigl(\AvoidA(u) \wedge \pre(\Ctx_{m_0,u}(0))\bigr)
      \rightarrow \post(\Ctx_{m_0,u}(0)).
  \end{aligned}
\]
For a step triple with target transition $t \in T$, we define
\[
  \begin{aligned}
    \ValidAdm((t,\pre,\post))
    \triangleq{}& \forall m_0 \in M,\forall u : \mathbb{N} \to I,\forall k \in \mathbb{N}.\\
    &\bigl(\AvoidA(u) \wedge t_u(k)=t \wedge \pre(\Ctx_{m_0,u}(k))\bigr)\\
    &\Longrightarrow \post(\Ctx_{m_0,u}(k+1)).
  \end{aligned}
\]
\end{definition}
...
The guard of $t$ is therefore exploited implicitly through
$\TransRel_t(\Ctx,\Ctx')$: a triple for $t$ is only required to hold on pairs
of contexts linked by an actual semantic step labeled by $t$. In other words,
the reduction reasons under the guard of $t$ by restricting attention to
realized steps, rather than by repeating that guard as a separate conjunct in
every precondition.
...
triples:
\begin{align*}
  \tripleinv{h^{\mathsf{init}}_{\NodeInv}} &\triangleq
    (\mathit{init},\top,\NodeInv_{\pi_S(p_0)}),\\
  \triplecoh{h^{\mathsf{init}}_{\Coherence}} &\triangleq
...
we define
\begin{align*}
  \tripleinv{h^{\mathsf{prop}}_{\NodeInv}(p)} &\triangleq
    \bigl(
      t,\
    ...
    \bigr),\\
  \triplecoh{h^{\mathsf{prop}}_{\Coherence}(p)} &\triangleq
    \bigl(
      t,\
...
the precondition of $\triplesafe{h^{\NoBad}(p)}$ holds at a tick, then no successor context
can exist, so the current context cannot realize the dangerous step $p$. The
two propagation families transport the two local facts needed by the safety
triples: user invariants and automaton coherence. These facts are therefore
...
\begin{align*}
  \Generated(h) \triangleq{}&
    h = \tripleinv{h^{\mathsf{init}}_{\NodeInv}}
    \ \vee\
    h = \triplecoh{h^{\mathsf{init}}_{\Coherence}}\\
  &\vee\ \exists p.\ \mathsf{WFStep}(p) \wedge h = \tripleinv{h^{\mathsf{prop}}_{\NodeInv}(p)}\\
  &\vee\ \exists p.\ \mathsf{WFStep}(p) \wedge h = \triplecoh{h^{\mathsf{prop}}_{\Coherence}(p)}\\
  &\vee\ \exists p.\ \mathsf{WFStep}(p) \wedge \Bad(p) \wedge h = \triplesafe{h^{\NoBad}(p)},
\end{align*}
and
\[
  \Generated_{\NoBad}(h) \triangleq
  \exists p.\ \mathsf{WFStep}(p) \wedge \Bad(p) \wedge h = \triplesafe{h^{\NoBad}(p)}.
\]
\end{definition}
...
\[
\begin{array}{rcl}
\tripleinv{h^{\mathsf{init}}_{\NodeInv}}
  &:& \text{initial user-invariant triple},\\
\triplecoh{h^{\mathsf{init}}_{\Coherence}}
  &:& \text{initial automaton-coherence triple},\\
\tripleinv{h^{\mathsf{prop}}_{\NodeInv}(p)}
  &:& \text{propagation of the user invariant along } p,\\
\triplecoh{h^{\mathsf{prop}}_{\Coherence}(p)}
...
\begin{tikzpicture}[
  every node/.style={font=\small},
  initbox/.style={draw=black!50, rounded corners=4pt, fill=coherencebg,
    minimum width=2.85cm, minimum height=0.92cm, align=center},
  propbox/.style={draw=black!50, rounded corners=4pt, fill=coherencebg,
    minimum width=2.35cm, minimum height=0.88cm, align=center},
  factbox/.style={draw=assumptionc!45!black, rounded corners=4pt, fill=assumptionbg,
    minimum width=2.45cm, minimum height=0.88cm, align=center},
  triplebox/.style={draw=guaranteec!55!black, rounded corners=4pt, fill=guaranteebg,
    minimum width=2.65cm, minimum height=0.88cm, align=center},
  arr/.style={-Latex, thick}
]
\node[initbox] (init) {initial tick\\$(\mathsf{Init},A_0,G_0)$};
\node[factbox, right=1.55cm of init, yshift=0.62cm] (initinv) {$\NodeInv_{\mathsf{Init}}$};
\node[factbox, right=1.55cm of init, yshift=-0.62cm] (initcoh) {$\CohCur_{(\mathsf{Init},A_0,G_0)}$};
\node[triplebox, right=1.55cm of initinv] (tinitinv) {$\tripleinv{h^{\mathsf{init}}_{\NodeInv}}$};
\node[triplebox, right=1.55cm of initcoh] (tinitcoh) {$\triplecoh{h^{\mathsf{init}}_{\Coherence}}$};
\draw[arr] (init) -- (initinv);
\draw[arr] (init) -- (initcoh);
\draw[arr] (initinv) -- (tinitinv);
\draw[arr] (initcoh) -- (tinitcoh);

\node[propbox, below=1.7cm of init, xshift=-1.45cm] (src) {$p_{\mathrm{src}}$\\$(s,q_A,q_G)$};
\node[propbox, right=2.0cm of src] (step) {product step $p$};
\node[propbox, right=2.0cm of step] (dst) {$p_{\mathrm{dst}}$\\$(s',q'_A,q'_G)$};
\draw[arr] (src) -- (step);
\draw[arr] (step) -- (dst);

\node[factbox, below=1.28cm of src, xshift=0.75cm] (srcfacts) {$\NodeInv_s \wedge \CohCur_{p_{\mathrm{src}}}$};
\node[factbox, below=1.28cm of dst, xshift=-0.75cm] (dstfacts) {$\NodeInv_{s'} \wedge \CohCur_{p_{\mathrm{dst}}}$};
\node[triplebox, below=1.28cm of step, yshift=0.48cm] (tpropinv) {$\tripleinv{h^{\mathsf{prop}}_{\NodeInv}(p)}$};
\node[triplebox, below=1.28cm of step, yshift=-0.62cm] (tpropcoh) {$\triplecoh{h^{\mathsf{prop}}_{\Coherence}(p)}$};
\draw[arr] (srcfacts) -- (tpropinv);
\draw[arr] (srcfacts) -- (tpropcoh);
\draw[arr] (tpropinv) -- (dstfacts);
\draw[arr] (tpropcoh) -- (dstfacts);
\end{tikzpicture}}
\caption{Initialization and propagation triples.}
\Description{A schematic view of the generated initialization and propagation triples. Two initialization triples establish invariant and coherence facts at the initial tick, and two propagation triples transport the same facts across one well-formed product step from a source state to a destination state.}
\label{fig:init-prop-triples}
\end{figure}
...
Figure~\ref{fig:init-prop-triples} complements
Figure~\ref{fig:dangerous-fragment} by making the non-dangerous part of the
reduction explicit. The upper half shows the two initialization triples
generated once from the initial product configuration. The lower half shows the
generic propagation pattern: for every well-formed product step, one triple
propagates the user invariant and one propagates product-state coherence. The
dangerous-step triple of Figure~\ref{fig:dangerous-fragment} is therefore not a
standalone object; it is the local safety layer placed on top of this
initialization-and-propagation discipline.
...
triples, together with semantic truth of the user invariants on admissible
executions, implies
conditional safety. The adequacy theorems explain why the
reduction is non-degenerate: under the appropriate semantic assumptions, the
generated triples are themselves admissibly valid. The proof proceeds through
a short chain of intermediate facts that correspond to the essential
conceptual steps of the reduction.

The proof boundary is also explicit. The semantic core assumes only the truth
of the generated triples and of user invariants on admissible executions when
proving soundness. For relative completeness, the mechanized result is
deliberately weaker: it establishes admissible-run validity, because global
correctness constrains only inputs satisfying $\AvoidA$. No statement in this
...
\noindent\leadcolor{Reading guide.}
...
For readability, we abbreviate the two global semantic assumptions used in the
rest of the section as follows:
\[
  \GlobCorr \triangleq
...
\[
  ...
  \AvoidA(u) \rightarrow \NodeInv_{s_u(k)}(\Ctx_{m_0,u}(k)).
\]
...
ingredient of the soundness and completeness arguments: user invariants are
supplied with the specification, so their truth on admissible executions cannot be
recovered from generated-triple validity alone.
...
\begin{proof}[Proof idea]
The precondition of $\triplesafe{h^{\NoBad}(p)}$ is exactly the conjunction of
matching, the source-state user invariant, and source-state coherence. These
three facts are hypotheses of the lemma. Moreover, realized steps are
well formed by Lemma~\ref{lem:realized-wf}. Since $\Bad(p)$ also holds, the
triple $\triplesafe{h^{\NoBad}(p)}$ is indeed one of the generated triples, and
its precondition is satisfied at the current tick.
\end{proof}

\subsection{Soundness}
The first result justifies the reduction. Automaton coherence is reconstructed
from the product semantics itself, whereas user invariants belong to the
specification and are therefore assumed semantically true on admissible
executions through $\InvTrue$.
...
\noindent\leadcolor{Proof idea.}
The proof proceeds by contradiction. A global guarantee violation first
localizes to one dangerous product step. Matching and automaton coherence then
show that the precondition of the corresponding safety triple holds on the very
tick that realizes this step; the remaining source-state invariant fact is
provided by $\InvTrue$. Since that safety triple is valid and its
postcondition is $\bot$, the dangerous tick cannot exist after all.
...
By $\InvTrue$ and the standing assumption $\AvoidA(u)$, the corresponding
user-invariant clause also holds at tick $k$:
\[
  \NodeInv_{\pi_S(p_{\mathrm{src}})}(\Ctx_{m_0,u}(k)).
\]
...
$\triplesafe{h^{\NoBad}(p)}$ hold at the current context. By
Lemma~\ref{lem:activate-nobad}, this safety triple is one of the generated
triples. By the first hypothesis
\[
  \forall h.\ \Generated(h) \rightarrow \Valid(h),
\]
the triple $\triplesafe{h^{\NoBad}(p)}$ is valid. Since its target is the concrete program
transition carried by $p$, triple validity applies to the concrete successor
context $\Ctx_{m_0,u}(k+1)$. But the postcondition of $\triplesafe{h^{\NoBad}(p)}$ is
...
guarantee violations localize to dangerous realized steps; automaton coherence
and true user invariants guarantee that the safety-triple precondition holds on
the dangerous tick; and a valid $\NoBad$ triple excludes any concrete
...
\paragraph{Intuition.}
Relative completeness runs in the opposite direction from soundness. Soundness
starts from valid generated triples and concludes conditional safety. Relative
completeness starts from semantic correctness of the program and asks whether
the reduction generates semantically legitimate local proof obligations. For
safety triples, this legitimacy follows directly from global correctness; for
triples that transport user invariants, one must additionally assume that the
user invariants are indeed true on admissible executions. The resulting notion is weaker
than classical relative completeness of Hoare logic
\cite{cook-1978}: we do not claim that every semantically valid local triple is
generated, nor that every valid generated triple is derivable in every
backend. We only claim that the triples produced by the reduction are
semantically valid on admissible runs under the natural hypotheses of the
model.

\begin{theorem}[Relative completeness of safety triples]
\label{thm:rc-nobad}
If the program is globally correct, then
every generated safety triple $\NoBad$ is admissibly valid.
\[
  \GlobCorr \Longrightarrow
  \forall h.\ \Generated_{\NoBad}(h) \rightarrow \ValidAdm(h).
\]
\end{theorem}

\noindent\leadcolor{Proof idea.}
If one generated safety triple were not admissibly valid, then some admissible
concrete execution would realize the dangerous step from which this triple was
extracted. But a dangerous realized step is exactly a local witness of global
guarantee violation under an admissible input. Global correctness therefore
rules out the failure of any generated safety triple on admissible runs.
...
\[
  p=(p_{\mathrm{src}},t,m,i,o,p_{\mathrm{dst}})
\]
such that $\mathsf{WFStep}(p)$, $\Bad(p)$, and $h = \triplesafe{h^{\NoBad}(p)}$.

We show that $h$ is admissibly valid. Suppose on the contrary that it is not.
Then, by definition of $\ValidAdm$, there exist $m_0 \in M$, an input stream
$u$, and a tick $k$ such that
\[
  \AvoidA(u),
  \qquad
  t_u(k)=t,
  \qquad
  \Match(\Ctx_{m_0,u}(k),p)
  \wedge \NodeInv_{\pi_S(p_{\mathrm{src}})}(\Ctx_{m_0,u}(k))
  \wedge \CohCur_{p_{\mathrm{src}}}(\Ctx_{m_0,u}(k))
\]
hold.
...
$A$, contradicting $\GlobCorr$ together with $\AvoidA(u)$. Therefore $h$ is
admissibly valid.
\end{proof}
...
correct and that the user invariants are semantically true on admissible
executions. Then every generated triple is admissibly valid.
\[
  \GlobCorr \wedge \InvTrue \Longrightarrow
  \forall h.\ \Generated(h) \rightarrow \ValidAdm(h).
\]
\end{theorem}
...
truth of user invariants on admissible executions. The safety family is handled by
...
If $h = \tripleinv{h^{\mathsf{init}}_{\NodeInv}}$, admissible validity follows
immediately from $\InvTrue$ at tick $0$.

If $h = \triplecoh{h^{\mathsf{init}}_{\Coherence}}$, admissible validity is
immediate from the definition of the initial product state and of the initial
tick context.

If $h = \tripleinv{h^{\mathsf{prop}}_{\NodeInv}(p)}$ for some well-formed step $p$, then
its admissible validity follows from $\InvTrue$: whenever the precondition
holds on an admissible current context, the postcondition is exactly the
invariant at the successor program state, which is true on the next tick of
the execution.

If $h = \triplecoh{h^{\mathsf{prop}}_{\Coherence}(p)}$ for some well-formed step $p$, then
its admissible validity follows from Lemmas~\ref{lem:matching},
\ref{lem:realized-wf}, and \ref{lem:coherence}: matching and current coherence
identify the current product state with $p_{\mathrm{src}}$, and structural
well-formedness of $p$ forces its target to coincide with the next product
state, so the postcondition $\CohCur_{p_{\mathrm{dst}}}$ holds on the
successor context.

Finally, if $h = \triplesafe{h^{\NoBad}(p)}$ for some dangerous well-formed step $p$, then
admissible validity follows from Theorem~\ref{thm:rc-nobad}. Thus all cases
covered by $\Generated(h)$ are admissibly valid, as claimed.
\end{proof}
...
reduction itself. Soundness states that valid generated triples, together with
true user invariants on admissible executions, are sufficient for global
conditional safety. Relative completeness states that, under the appropriate
semantic hypotheses, the reduction does not generate arbitrary false local
proof objects on admissible runs. This is the
mathematically relevant notion of adequacy for the reduction layer,
independently of any later backend proof search.
...
\text{non-reset tick from Run} &\leadsto \text{ the same propagation triples, plus possibly } \triplesafe{h^{\NoBad}(p)}.
\end{aligned}
\]
This is the concrete reflection of the abstract chain
\[
\text{global violation} \Longrightarrow \text{dangerous step}
\Longrightarrow \NoBad\text{ clause} \Longrightarrow \triplesafe{h^{\NoBad}(p)}.
\]
...
Z3 through Why3 transformations. Using Why3 with the
\texttt{simplify\_formula}, \texttt{eliminate\_if\_term},
\texttt{remove\_unused}, and \texttt{split\_vc} transformations, a
\texttt{30s} timeout per goal, Why3~1.8.2, Z3~4.15.2, and the local development
machine (Apple M2, 8 cores, 16~GB RAM, macOS 15.6 / Darwin~24.6.0), the full
generated file is discharged in about \texttt{4.4s} of wall-clock time. This
timing is only a reproducibility indicator for the running example, not a
performance claim about the method. It does not strengthen the abstract
...
\subsection{A more data-intensive Kairos example}
The running example remains intentionally small. This is useful for exposition,
but it does not fully illustrate one practical strength of the reduction:
the program state itself need not be finite. To make this point concrete, the
current Kairos prototype also verifies the following \texttt{credit\_balance\_monitor}
node, whose control structure is tiny but whose semantic state ranges over
unbounded integers:

\begin{lstlisting}[style=kairosstyle]
node credit_balance_monitor(reset: int, credit: int, debit: int)
returns (bal: int)
contracts
  requires: G((reset = 0 or reset = 1)
           and (credit >= 0) and (debit >= 0));
...
This node is still a safety problem, but it is no longer naturally viewed as a
finite-state verification instance. The balance evolves over the integer domain
without any a priori bound, so explicit-state model checking would require a
separate abstraction argument before it could even recover a finite state
space. By contrast, the reduction of the present paper keeps the automaton side
finite while leaving the program memory symbolic. The user invariant
\lstinline|(m >= 0) and (m = prev bal)| provides the local arithmetic summary
needed to show that every reachable non-reset transition preserves
\lstinline|bal >= 0| under the transition guard.

Conceptually, this second example does not change the proof architecture at
all. The program still induces an explicit product with assumption and
guarantee automata, dangerous steps still localize global guarantee violations,
and the resulting obligations are still discharged as local transition proofs.
What changes is only the character of the program state: the example stresses
that the method is not tied to finite-state program semantics even though the
specification side continues to use finite safety automata.
...
particular tool chain. In its current form it contains eight files and 1217
non-comment, non-empty lines. Counting lines outside proof blocks as
specification and lines from \texttt{Proof.} to \texttt{Qed.}/\texttt{Defined.}
as proof, the development contains 730 specification lines and 487 proof lines.
...
results. The file \texttt{ResettableDelayExample.v} instantiates the abstract
theory on the running example and concludes \texttt{resettable\_delay\_correct}
by proving the generated triples valid and then applying the abstract soundness
theorem.
...
paper keeps the proof ideas in the main text and only records the paper-to-Rocq
correspondence here:

\begin{center}
\begin{tabular}{lrr}
\textbf{Rocq file} & \textbf{Spec} & \textbf{Proof} \\
\hline
\texttt{ReactiveModel.v} & 73 & 12 \\
\texttt{ConditionalSafety.v} & 53 & 23 \\
\texttt{ExplicitProduct.v} & 153 & 40 \\
\texttt{GeneratedClauses.v} & 69 & 0 \\
\texttt{RelationalTriples.v} & 115 & 13 \\
\texttt{Soundness.v} & 34 & 46 \\
\texttt{RelativeCompleteness.v} & 51 & 89 \\
\texttt{ResettableDelayExample.v} & 182 & 264 \\
\hline
\textbf{Total} & \textbf{730} & \textbf{487}
\end{tabular}
\end{center}

Overall, this yields 1217 non-comment, non-empty lines split into 730
specification lines and 487 proof lines.
...
the same organization as the paper. The concrete example file is the only place
where the abstract theory is instantiated on one fixed program; the remaining
files stay fully parametric.
...
completeness of the safety reduction on admissible runs, and relative
completeness of all generated triples on admissible runs under true user
invariants. The theory is independent of any particular tool. Concrete
compilers, encodings, and solver backends refine this core but do not define
it.
...
instantiations. A first direction is to widen the specification side while
preserving the same local-proof interface, for instance by clarifying which
larger safety fragments still compile to automata and generated triples in a
principled way. A second direction is to tighten the bridge to proof-producing
backends, so that the semantic clauses and generated triples of the paper map
more directly to concrete verification conditions and proof certificates. A
third direction is to systematize language-level embeddings, in particular for
synchronous languages with richer state structure than the running example,
while keeping the abstract reduction independent of any one source language.
