...
(** * Soundness
    ...
    This module proves the main theorem of the development. The assumptions are
    exactly the semantic obligations isolated in the paper:

    - every generated triple is semantically valid;
    - user invariants are true on admissible runs.

    Under these hypotheses, the program is conditionally safe. *)
...
Section Soundness.
  ...
  (** Public formulation of conditional correctness used in this file. *)
  Definition globally_correct : Prop :=
    forall u,
      AvoidA Spec u ->
      AvoidG Spec u.

  (** Hypothesis stating that all generated local proof obligations are valid. *)
  Hypothesis GeneratedTripleValid :
    forall ht : @RelHoareTriple P Spec,
      @GeneratedTriple P Spec node_inv ht ->
      @TripleValid P Spec ht.

  (** Hypothesis stating that the user invariant family is semantically true on
  ...
  (** On any concrete run, current coherence with the current product state is
  ...
  (** The current concrete context matches the selected step extracted from the
      same run and tick. *)
  ...
  (** If a selected concrete step were dangerous on an admissible run, then the
      generated [NoBad] triple for this step would be valid and activated,
  ...
  (** Main soundness theorem of the reduction. *)
