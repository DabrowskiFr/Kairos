...
let rec nnf_ltl ?(neg = false) (f : fo_ltl) : fo_ltl =
  ...
  | LW (a, b) ->
      if neg then
        let msg =
          "NNF: negation above W not supported in safety fragment: not (" ^ Support.string_of_ltl a
          ^ " W " ^ Support.string_of_ltl b ^ ")"
        in
        failwith msg
      else LW (nnf_ltl a, nnf_ltl b)
...
let rec simplify_ltl (f : fo_ltl) : fo_ltl =
  ...
  | LW (a, b) ->
      let a = simplify_ltl a in
      let b = simplify_ltl b in
      if b = LTrue then LTrue
      else if a = LFalse then b
      else if b = LFalse then LG a
      else if a = LTrue then LTrue
      else LW (a, b)
  | _ -> f
